...
describe("hydrate", () => {
  it("refuses a Space the device did not declare in hello", async () => {
    ...
    await core.handleMessage(a, frame({ t: "publish", records: [makeRecord({ spaceId: "s1", hlc: makeHlc(10, "dev-a") })] }), []);

    await core.handleMessage(b, frame({ t: "hydrate", spaceId: "s1", sinceHlc: null }), [a]);
    expect(b.ofType("records")).toHaveLength(0);
    expect(b.ofType("hydrate.done")).toHaveLength(0);
    expect(b.ofType("error")[0]).toMatchObject({ code: "malformed", message: "space not declared in hello" });
  });

...
describe("agreement keys (D6)", () => {
  it("exportPublicKeyRaw yields 32 raw bytes for both Ed25519 and X25519 keys", async () => {
    const signing = await generateDeviceKeypair();
    const agreement = await generateAgreementKeypair();
    expect((await exportPublicKeyRaw(signing.publicKey)).length).toBe(32);
    const raw = await exportPublicKeyRaw(agreement.publicKey);
    expect(raw.length).toBe(32);
    const imported = await importAgreementPublicKeyRaw(raw);
    expect(imported.algorithm.name).toBe("X25519");
    expect(await exportPublicKeyRaw(imported)).toEqual(raw);
    // A signing key is not an agreement key and vice versa.
    await expect(importPublicKeyRaw(raw).then((k) => verifyRecord(k, new Uint8Array(64), fields))).resolves.toBe(false);
  });
});

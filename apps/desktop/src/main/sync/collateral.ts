/**
 * Pure collateral-damage logic for tombstone removal (§10.2 applier).
 *
 ...
 * and reconstruction go through the exact same mapping the protocol's
 * cookie-map uses. No Electron imports — unit tests exercise this directly.
 */
...
/** An Electron cookie's identity; never partitioned, so never null. */
function electronIdentity(spaceId: string, cookie: ElectronCookie): CookieIdentity {
  const identity = identityForCookie(spaceId, portableCookieFromElectron(cookie));
  if (identity === null) throw new Error("an Electron cookie is never partitioned");
  return identity;
}
...
function scopeKeyForCookie(cookie: ElectronCookie): string {
  const identity = electronIdentity("", cookie);
...
export function restoreDetailsFor(
  ...
  return setDetailsForPlain({
    identity: electronIdentity(spaceId, cookie),
    attributes: attributesForCookie(portableCookieFromElectron(cookie)),
    deleted: false,
  });
}

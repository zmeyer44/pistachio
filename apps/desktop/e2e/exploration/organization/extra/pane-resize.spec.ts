...
import { sidebarMenuItem } from "../../../tests/footer";
...
test("resizing a split pane reflows the pages during the drag, not after it", async () => {
  ...
  try {
    ...
    await (await sidebarMenuItem(shell, "split-toggle")).click();
    ...
    await captureComposite(app,"resize-during-drag");
    await shell.mouse.up();
    await captureComposite(app,"resize-after-release");
    ...
    await captureComposite(app,"pane-resize-final").catch(()=>{});
    await app.close();
  }
});

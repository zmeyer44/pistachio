...
import { shellPage } from "../../../tests/windows";
...
test("a split group grows to four panes and switches between grid, vertical, and horizontal layouts", async () => {
  ...
  try {
    ...
    await captureComposite(app,"four-pane-grid");
    ...
    await captureComposite(app,"four-pane-vertical");
    ...
    await captureComposite(app,"four-pane-horizontal");
    ...
    await captureComposite(app,"four-pane-split-final").catch(()=>{});
    await app.close();
  }
});
